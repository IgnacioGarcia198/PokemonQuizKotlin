...
class PokemonDetailViewModelTest {
    ...
    fun stubSharedPrefInitial() {
        ...
        whenever(mockPreferences.getLong(HomeViewModel.BASE_POK_DAY, 0)).doReturn(0)
        whenever(mockEditor.putLong(any(), any())).doReturn(mockEditor)
    }

    fun stubSharedPrefSavedValue() {
        whenever(mockPreferences.edit()).doReturn(mockEditor)
        whenever(mockEditor.putLong(any(), any())).doReturn(mockEditor)
        ...
        whenever(mockPreferences.getLong(HomeViewModel.BASE_POK_DAY, 0)).doReturn(c.timeInMillis)
    }
    ...
    fun initPushGetsRightPokDetailId() {
        val thecallback = {
            ...
            viewModel.onFlavorTextAndNameResult(versions, flavorAndName)
        }
        //val thecallback(versions : List<String>)
        coroutinesTestRule.testDispatcher.runBlockingTest {
            whenever(repository.getFlavorTextAndNameFirstTime(1, newPokemonCallback = thecallback)).doAnswer {
                viewModel.onFlavorTextAndNameResult(theversions, theflavorAndName)
            }
        }
        viewModel.initPush(1)
        //viewModel.onFlavorTextAndNameResult()

        viewModel.currentIdLiveData.observeOnce {
            assert(it == 1)
        }
    ...
    @Test
    fun calculateNewIdTestInitiallyPokemonDetail() {

        viewModel.calculateNewId(1)

        viewModel.currentIdLiveData.observeOnce {
            assert(it == 1)
        }

        viewModel.dailyOrDetail.observeOnce {
            assert(it)
        }
    ...
    @Test
    fun calculateNewIdTestWithSameIdPokemonDetail() {
        viewModel.calculateNewId(1)
        ...
        viewModel.calculateNewId(1)
        assert(viewModel.inited)
        viewModel.currentIdLiveData.test()
            .assertHasValue()
            .assertHistorySize(1)
            .assertValueHistory(1)
            .assertValue(1)
        viewModel.currentIdLiveData.observeOnce {
            assert(it == 1)
        }
    ...
    @Test
    fun calculateNewIdTestWithDifferentPokemonDetail() {
        viewModel.calculateNewId(1)
        assert(!viewModel.inited)
        viewModel.inited = true
        viewModel.currentIdLiveData.test()
        ...
        viewModel.inited = true
        viewModel.calculateNewId(2)
        assert(!viewModel.inited)
        viewModel.currentIdLiveData.test()
            .assertHasValue()
            .assertHistorySize(1)
            .assertValueHistory(2)
            .assertValue(2)
        viewModel.currentIdLiveData.observeOnce {
            assert(it == 2)
        }
    ...
    @Test
    fun calculateNewIdDailyPokemon() {
        stubSharedPrefInitial()
        viewModel.calculateNewId(0)
        assert(!viewModel.inited)
        viewModel.currentIdLiveData.test()
            .assertHasValue()
            .assertHistorySize(1)
            .assertValue(viewModel.calculateTodayPokId())
        viewModel.currentIdLiveData.observeOnce {
            assert(it == viewModel.calculateTodayPokId())
        }
    }

    @Test
    fun calculateNewIdTestWithSameIdDailyPokemon() {
        stubSharedPrefInitial()
        viewModel.calculateNewId(0)
        assert(!viewModel.inited)
        viewModel.inited = true
        viewModel.currentIdLiveData.test()
            .assertHasValue()
            .assertHistorySize(1)
            .assertValueHistory(viewModel.calculateTodayPokId())
            .assertValue(viewModel.calculateTodayPokId())
        viewModel.currentIdLiveData.observeOnce {
            assert(it == viewModel.calculateTodayPokId())
        }
    ...
    @Test
    fun calculateNewIdTestWithDailyAndDetail() {
        stubSharedPrefInitial()
        println("currentId is ${viewModel.currentId}, new Id is 0")
        viewModel.calculateNewId(0)
        assert(!viewModel.inited)
        viewModel.inited = true
        viewModel.currentIdLiveData.test()
            ...
            .assertValue(viewModel.calculateTodayPokId())
        viewModel.currentIdLiveData.observeOnce {
            println("currentId is ${viewModel.currentId}")
            assert(it == viewModel.calculateTodayPokId())
        }
        viewModel.dailyOrDetail.observeOnce {
            assert(!it)
        ...
        //viewModel._currentIdLiveData.value = -1
        //viewModel.inited = true

        stubSharedPrefSavedValue()

        println("currentId is ${viewModel.currentId}, new Id is ${viewModel.calculateTodayPokId()}")

        viewModel.calculateNewId(viewModel.calculateTodayPokId())
        assert(!viewModel.inited)
        viewModel.currentIdLiveData.test()
            .assertHasValue()
            .doOnChanged { println("the id has changed") }
            .assertHistorySize(1)
            .assertValueHistory(viewModel.calculateTodayPokId())
            .assertValue(viewModel.calculateTodayPokId())
        viewModel.dailyOrDetail.observeOnce {
    ...
    @Test
    fun calculateNewIdTestWithDetailAndDaily() {
        stubSharedPrefSavedValue()
    ...
    fun getFlavorAndNameInitiallyTest() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            whenever(
                ...
                val argument = it.arguments[3]
                val thecall = argument as ((List<String>,Pair<String,String>) ->Unit)
                thecall.invoke(theversions, theflavorAndName)
                //viewModel.onFlavorTextAndNameResult(theversions, theflavorAndName)
            }
        }
        viewModel.currentId = 1
        viewModel.getFlavorAndNameInitially()
        ...
        viewModel.versionList.observeOnce {
    ...
    fun onFlavorTextAndNameResultCallsItsFunctions() {
        ...
        spyOfTestedClass.onFlavorTextAndNameResult(theversions, theflavorAndName)
        verify(spyOfTestedClass).onVersionsReady(theversions)
        verify(spyOfTestedClass).setFlavorAndName(theflavorAndName)
        assert(spyOfTestedClass.inited)

    }

    @Test
    fun onVersionsReadyTest() {
        viewModel.onVersionsReady(theversions)

        viewModel.versionList.observeOnce {
            assert(it == theversions)
        }
        viewModel.version.observeOnce {
            assert(it == theversions.first())
        }
    }
    ...
    fun setFlavorAndNameTest() {
        ...
        viewModel.name.observeOnce {
            assert(it == theflavorAndName.second)
        }
        viewModel.flavorText.observeOnce {
            assert(it == theflavorAndName.first)
        }
    }
    ...
    @Test
    fun initPushTest() {
        //val onFlavorTextAndNameResult = viewModel::onFlavorTextAndNameResult
        ...
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val captor1 = argumentCaptor<(List<String>, Pair<String, String>) -> Unit>()
            val captor2 = argumentCaptor<Int>()
            val captor3 = argumentCaptor<String>()
            val captor4 = argumentCaptor<String>()

            //spyOfTestedClass.onFlavorTextAndNameResult(theversions, theflavorAndName)
            verify(spyOfTestedClass, times(1)).onFlavorTextAndNameResult(theversions, theflavorAndName)
            verify(repository).getFlavorTextAndNameFirstTime(
                captor2.capture(),captor3.capture(),captor4.capture(),captor1.capture()
            )
            //verify(repository).changeResponseState(PokemonResponseState.DONE)
            ...
            val function = captor1.firstValue
            function.invoke(theversions, theflavorAndName)
            verify(spyOfTestedClass, times(1)).onFlavorTextAndNameResult(theversions, theflavorAndName)
